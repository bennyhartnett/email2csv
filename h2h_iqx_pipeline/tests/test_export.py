...
def test_export_writes_files_and_reorders(tmp_path):
    combo_df = pd.DataFrame({"Source": ["IBEW D4"], "Email": ["a@example.com"]})
    ...
    config = {
        ...
        "iqx_import": {"column_order": ["Email", "Source"]},
    }

    paths = export.write_outputs(month="2025-12", combo_df=combo_df, dedup_df=dedup_df, config=config)
    ...
    assert csv_content.splitlines()[0] == "Email,Source"
